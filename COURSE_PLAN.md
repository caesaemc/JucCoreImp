...
当前环境的默认 JDK 是 17。正式开始课程前可以先完成大部分基础课程；第 13 课虚拟线程需要 JDK 21 或更高版本。
...
│   └── ...
├── src/
│   ├── main/java/com/example/juclearn/
│   │   ├── lesson01/
│   │   ├── lesson02/
│   │   └── ...
│   └── test/java/com/example/juclearn/
│       ├── lesson01/
│       ├── lesson02/
│       └── ...
├── benchmarks/
...
1. 是否接受 8 周 16 课的规模。
2. 是否采用 JDK 21 作为课程主版本。
3. 是否保留第 16 课的综合项目。
4. 是否接受“先做错误实验，再学习正确实现”的授课方式。
